...
- **CI/CD**: Added smoke test job to both GitLab CI and GitHub Actions
  ...
  - Uses `examples` client with `example.ts` test
...
### Fixed
- **Configuration Validation**: Fixed issue where validation errors were suppressed in `run-test.sh`
  - Removed stderr redirection to `/dev/null`
