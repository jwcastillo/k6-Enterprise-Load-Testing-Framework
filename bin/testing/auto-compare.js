...
// Find all summary JSON result files (generated by --summary-export)
const allResults = globSync(path.join(reportsDir, 'k6-summary-*.json'))
  .map(file => ({
    path: file,
 ...
 * Summary files (--summary-export) are simple JSON objects with aggregated metrics
 */
function parseK6Output(filePath) {
  try {
    ...
    throw new Error(`Failed to parse k6 summary from ${filePath}: ${e.message}`);
  }
}

// Load and parse results
const current = parseK6Output(currentResult.path);
const baselines = resultsToCompare.map(r => ({
  name: r.name,
  data: parseK6Output(r.path),
