...
export class Runner {
  ...
  public async run() {
    console.log(
      `Starting Runner for client: ${this.options.client} on env: ${this.options.env}`
    );
    ...
    if (!fs.existsSync(clientDir)) {
      ...
      const clientsDir = path.join(this.rootDir, "clients");
      const availableClients = fs.existsSync(clientsDir)
        ? fs.readdirSync(clientsDir).filter((name) => {
            const stat = fs.statSync(path.join(clientsDir, name));
            return stat.isDirectory() && !name.startsWith(".");
          })
        : [];

      console.error(`\n❌ Client '${this.options.client}' not found`);
      console.error(`   Directory does not exist: ${clientDir}\n`);
      ...
      if (availableClients.length > 0) {
        ...
        console.error(
          `   Create a new client directory with: mkdir -p clients/your-client/scenarios`
        );
      }

      throw new Error(`Client '${this.options.client}' not found`);
    }
    ...
    if (this.options.test) {
      const possiblePaths = [
        path.join(
          ...
          this.options.client,
          "scenarios",
          this.options.test.replace(".ts", ".js")
        ),
    ...
    if (!scriptPath) {
      throw new Error(
        `Test script not found. Searched in: ${clientDir}/scenarios and dist/`
      );
    }

    // 4. Setup report directories and paths
    const testName =
      this.options.test?.replace(".ts", "").replace(".js", "") || "test";
    ...
    const webDashboardPath = path.join(
      reportDir,
    ...
    const summaryMode =
      k6Options.summaryMode || process.env.K6_SUMMARY_MODE || "full";
    k6Args.push("--summary-mode", summaryMode);

    if (k6Options.summaryTrendStats) {
      k6Args.push(
        "--summary-trend-stats",
        k6Options.summaryTrendStats.join(",")
      );
    }
    ...
    try {
      ...
      const enterpriseReportPath = path.join(
        reportDir,
      ...
      if (
        ...
        console.log(
          "   Run: brew install k6 (Mac) or see https://k6.io/docs/get-started/installation/"
        );
  ...
  private async loadConfig(clientDir: string) {
    ...
    const clientDefaults = fs.existsSync(clientDefaultsPath)
      ? await fs.readJson(clientDefaultsPath)
      : {};
    ...
    const clientEnvPath = path.join(
      clientDir,
      "config",
      `${this.options.env}.json`
    );
    const clientEnv = fs.existsSync(clientEnvPath)
      ? await fs.readJson(clientEnvPath)
      : {};
    ...
    if (process.env.K6_SUMMARY_TREND_STATS) {
      ...
      mergedConfig.k6Options.summaryTrendStats =
        process.env.K6_SUMMARY_TREND_STATS.split(",");
    }
    ...
    if (process.env.K6_METRICS_BACKENDS) {
      mergedConfig.k6Options = mergedConfig.k6Options || {};
      const backends = process.env.K6_METRICS_BACKENDS.split(",").map(
        (type) => ({ type: type.trim() })
      );
